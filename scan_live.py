        ...
        html = h_wrap("测试 · 板块共振信号", ["邮件通道正常，这是 HTML 排版样例"],
                      [h_section(f"电子设备 {h_pct(3.65)}", "91% 上涨 · 561 只"),
                       h_card(h_name("德明利", "001309"), h_pct(7.19),
                              ...
                              h_kv(("首次突破", "10:44"), ("20日均额", "99.0 亿"), ("距60日高", "-56%")) +
                              h_buy_btn("001309", None, "电子设备", "测试邮件"), "前期龙头")] +
                      ([f"<div style='margin-top:16px;text-align:center'><a href='{web_base()}/?k={quote(load_env().get('WEB_TOKEN', ''))}' "
                        f"style='color:{GRAY};font-size:13px'>查看我的持仓 / 冲高监控状态</a></div>"] if web_base() else []),
                      "卖出规则：次日 10:00 前离场，早盘冲高即走。上面的「我已买入」可以直接点，会真的记一笔德明利的持仓；"
                      "不想要就在持仓页点「点错了，删除」。")
