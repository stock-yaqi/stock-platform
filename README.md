...
盘面形态分不清吸筹和出货（回测无效），股东户数是唯一的硬证据。东财 F10 股东户数 2024-03 ~ 2026-09、5211 只、每期期末价、相对全市场同期超额：

| 条件 | 下一期超额 | 披露后再下一期 | 按期为正 |
|---|---|---|---|
| 户数环比下降 ≥15% | +2.82% | +1.53% | 11/15 |
| 户数环比下降 ≥10% | +2.04% | +1.14% | 15/20 |
| 连续两期下降 ≥5% | +1.71% | +0.35% | 11/16 |
| 户数下降 ≥5% 且前十大占比上升 | +2.22% | +0.27% | 7/8 |
| 户数环比上升 ≥20%（散户涌入） | -2.50% | -0.60% | 8/18 |

得分 0-4：最新一期降 ≥10%、连续两期降 ≥5%、前十大占比上升、低位缩量；户数增 ≥20% 标回避。季度数据滞后 1-2 个月，是中期筹码线索。
...
输出 `筹码集中_<日期>.csv`、`户数大增回避_<日期>.csv`，每周一随 daily_job 发一封。
